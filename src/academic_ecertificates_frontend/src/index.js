...
import {
  canisterId,
  createActor,
} from "../../declarations/academic_ecertificates_backend";
...
function redirect(path, params) {
  ...
  myUrlWithParams.searchParams.append(
    "canisterId",
    process.env.CANISTER_ID_ACADEMIC_ECERTIFICATES_FRONTEND
  );
  if (params) {
  ...
  if (resp.statusCode === BigInt(200)) {
    ...
    $("i[data-id]").on("click", async function (e) {
      ...
      if (resp.statusCode == BigInt(200)) {
        var baseurl =
          process.env.DFX_NETWORK === "ic"
            ? `https://${process.env.CANISTER_ID_ACADEMIC_ECERTIFICATES_FRONTEND}.icp0.io/?cert=`
            : `http://${process.env.CANISTER_ID_ACADEMIC_ECERTIFICATES_FRONTEND}.localhost:4943/?cert=`;
        var universityUrl =
          process.env.DFX_NETWORK === "ic"
            ? `https://${process.env.CANISTER_ID_ACADEMIC_ECERTIFICATES_FRONTEND}.icp0.io/university.html?principal=`
            : `http://${process.env.CANISTER_ID_ACADEMIC_ECERTIFICATES_FRONTEND}.localhost:4943/university.html?principal=`;
...
async function universityinit() {
  ...
  if (url.searchParams.has("principal")) {
    ...
    var resp = await academic_ecertificates_actor.getUniversity([
      Principal.fromText(universityPrincipal),
    ]);
    console.log(resp);
    if (resp.statusCode === BigInt(200)) {
      var university = resp.university[0];
